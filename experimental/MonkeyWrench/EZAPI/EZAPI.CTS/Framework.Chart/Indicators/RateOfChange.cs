...
namespace EZAPI.Framework.Chart.Indicators
{
    public class RateOfChange : Indicator
    {
        ...
        public RateOfChange(string name = "Rate Of Change", string description = "", string moreInfoWebPage = "")
            : base(name)
        {
            ...
            if (description == "")
                this.description = "The rate of change formula calculates the rate of price change compared with historical data. It differs from the Performance Formula in that it calculates the rate of change against a period of days prior to the current price. The output is a percentage. See also: Volume Rate of Change formula.";
            if (moreInfoWebPage == "")
        ...
        public override void Draw(MSChart.Chart chart1, bool enable = true)
        {
            ...
            chart1.DataManipulator.FinancialFormula(MSChart.FinancialFormula.RateOfChange, formulaParameters, "candlestickSeries:Y3", INDICATOR + ":Y");
