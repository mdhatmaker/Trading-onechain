...
namespace EZAPI.Framework.Chart.Indicators
{
    public class VolumeRateOfChange : Indicator
    {
        ...
        public VolumeRateOfChange(string name = "Volume Rate Of Change", string description = "", string moreInfoWebPage = "")
            : base(name)
        {
            ...
            if (description == "")
                this.description = "The volume rate of change formula calculates the rate of volume change compared with historical data. It differs from the Performance Formula in that it calculates the rate of change against a period of days prior to the current volume. The output is a percentage.";
            if (moreInfoWebPage == "")
                this.moreInfoWebPage = "http://stockcharts.com/school/doku.php?id=chart_school:technical_indicators:rate_of_change";

            // ADD PARAMETERS HERE
            AddParameter(new IndicatorParameter<int>("Period", "Number of bars prior to the current bar. This formula uses the data from that bar as a reference when calculating the current rate of change.", 10));
        ...
        public override void Draw(MSChart.Chart chart1, bool enable = true)
        {
            ...
            string INDICATOR = "VolumeRateOfChange";
            ...
            chart1.DataManipulator.FinancialFormula(MSChart.FinancialFormula.RateOfChange, formulaParameters, "candlestickVolumeSeries:Y", INDICATOR + ":Y");
