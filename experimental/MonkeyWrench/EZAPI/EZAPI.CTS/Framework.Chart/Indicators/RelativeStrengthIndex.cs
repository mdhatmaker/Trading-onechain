...
namespace EZAPI.Framework.Chart.Indicators
{
    public class RelativeStrengthIndex : Indicator
    {
        ...
        public RelativeStrengthIndex(string name = "Relative Strength Index", string description = "", string moreInfoWebPage = "")
            : base(name)
        {
            ...
            if (description == "")
                this.description = "The relative strength index formula is a momentum oscillator formula that compares upward movements of the close price with downward movements, and outputs values from 0 to 100. A value close to 100 indicates that the price is about to move downward, and a value close to 0 indicates that the price is about to move upward.";
            if (moreInfoWebPage == "")
                this.moreInfoWebPage = "http://stockcharts.com/school/doku.php?id=chart_school:technical_indicators:relative_strength_index_rsi";

            // ADD PARAMETERS HERE
            AddParameter(new IndicatorParameter<int>("Period", "Period for calculating the relative strength index.", 10));
        ...
        public override void Draw(MSChart.Chart chart1, bool enable = true)
        {
            ...
            string INDICATOR = "RelativeStrengthIndex";
            ...
            chart1.DataManipulator.FinancialFormula(MSChart.FinancialFormula.RelativeStrengthIndex, formulaParameters, "candlestickSeries:Y3", INDICATOR + ":Y");
