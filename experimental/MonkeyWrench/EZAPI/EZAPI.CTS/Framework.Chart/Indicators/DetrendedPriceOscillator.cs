...
namespace EZAPI.Framework.Chart.Indicators
{
    ...
    {
        public DetrendedPriceOscillator(string name = "Detrended Price Oscillator", string description = "", string moreInfoWebPage = "")
            : base(name)
        {
            ...
            if (description == "")
                this.description = "The detrended price oscillator formula calculates the difference between the daily price and the moving average. This is useful for identifying cycles and overbought and oversold price levels.";
            if (moreInfoWebPage == "")
                this.moreInfoWebPage = "http://stockcharts.com/school/doku.php?id=chart_school:technical_indicators:detrended_price_osci";

            // ADD PARAMETERS HERE
            AddParameter(new IndicatorParameter<int>("Period", "Length (in bars) for calculating the moving average for the detrended price oscillator index.", 10));
        ...
        public override void Draw(MSChart.Chart chart1, bool enable = true)
        {
            ...
            string INDICATOR = "DetrendedPriceOscillator";
            ...
            chart1.DataManipulator.FinancialFormula(MSChart.FinancialFormula.DetrendedPriceOscillator, formulaParameters, "candlestickSeries:Y3", INDICATOR + ":Y");
