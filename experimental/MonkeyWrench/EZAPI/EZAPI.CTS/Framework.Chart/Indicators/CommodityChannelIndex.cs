...
namespace EZAPI.Framework.Chart.Indicators
{
    ...
    {
        public CommodityChannelIndex(string name = "Commodity Channel Index", string description = "", string moreInfoWebPage = "")
            : base(name)
        {
            ...
            if (description == "")
                this.description = "The commodity channel index formula calculates the mean deviation of the daily average price from the moving average. A value above 100 indicates that the commodity is overbought, and a value below -100 indicates that the commodity is oversold.";
            if (moreInfoWebPage == "")
                this.moreInfoWebPage = "http://stockcharts.com/school/doku.php?id=chart_school:technical_indicators:commodity_channel_index_cci";

            // ADD PARAMETERS HERE
            AddParameter(new IndicatorParameter<int>("Period", "Length (in bars) for calculating the commodity channel index.", 10));
        ...
        public override void Draw(MSChart.Chart chart1, bool enable = true)
        {
            ...
            string INDICATOR = "CommodityChannelIndex";
            ...
            chart1.DataManipulator.FinancialFormula(MSChart.FinancialFormula.CommodityChannelIndex, formulaParameters, "candlestickSeries:Y1,candlestickSeries:Y2,candlestickSeries:Y3", INDICATOR + ":Y");
