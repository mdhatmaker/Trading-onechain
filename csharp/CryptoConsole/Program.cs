...
using CryptoRestApis.RestApi;
using CryptoRestApis.RestApi.Models;
using CryptoRestApis.ExchangeX.CoinMarketCap;
using CryptoRestApis.Exchange.HitBtc;
...
namespace CryptoConsole
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
			...
			Console.WriteLine("********** GATOR " + new string('*', 103));
            ...
            if (args.Length < 3)
			{
				Console.WriteLine("usage: dotnet CryptoConsole.dll <symbol> <amount> <bips> (#display)");
				Console.WriteLine();
				Console.WriteLine("   ex: dotnet CryptoConsole.dll ETH-USD 200 125");
				Console.WriteLine("       dotnet CryptoConsole.dll BTC-USD 90 125 20");
				Console.WriteLine("       dotnet CryptoConsole.dll BTC-USDT 75 150");
				return;
			}

			string symbol = args[0];
			decimal amount = decimal.Parse(args[1]);
			int bips = int.Parse(args[2]);
			int displayBook = 0;
			if (args.Length > 3)
				displayBook = int.Parse(args[3]);
